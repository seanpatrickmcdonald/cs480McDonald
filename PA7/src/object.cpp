...
GLuint loadBMP()
{
    ...
    BMPLoader loader;
    loader.loadFromFile("../assets/earthmap1k.bmp");
...
void Object::readFromConfig(char* configPath, ObjInit obj)
{
	...
	{
		configFile >> obj_name[index];
		//check for invalid
		configFile >> obj_filename[index];
		//check fr invalid
		configFile >> tex_filename[index];
		//check for invalid
		configFile >> parent_name[index];
		//check fr invalid
		configFile >> rotationSpeed[index];
		//check for invalid
		configFile >> orbitSpeed[index];
		//check fr invalid
		configFile >> orbitRadius[index];
	}
